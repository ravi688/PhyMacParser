/*
    Every PipelineLayout:
        1. Will have global set layout at set number GLOBAL_SET (which is 0)            -- Constant always
        2. Will have render set layout at set number RENDER_SET (which is 1)            -- Variable
        3. Will have sub render set layout at set number SUB_RENDER_SET (which is 2)    -- Variable
        4. Will have material set layout at set number MATERIAL_SET (which is 3)        -- Variable
        5. Will have object set layout at set number OBJECT_SET (which is 4)            -- Constant

    Number of Sub passes == Number of Graphics Pipelines
  */
...
Shader
{
    ...
    Properties
    {
        ...
        fragment vertex [push_constant] [0] uniform GameData
        {
            float time;
        } gameData;
    }
    ...
    RenderPass
    {
        ...
        SubPass
        {
            ...
            glsl
            {
                ...
                layout(input_attachment_index = 0, set = SUB_RENDER_SET, binding = INPUT_ATTACHMENT_BINDING0) uniform subpassInput previousColor;
...
Shader
{
    ...
    {
        per-vertex [0, POSITION_LOCATION] vec3 position;
        per-vertex [0, NORMAL_LOCATION] vec3 normal;
        per-vertex [0, COLOR_LOCATION] vec4 color;
        per-vertex [0, TANGENT_LOCATION] vec3 tangent;
        per-vertex [0, TEXCOORD_LOCATION] vec2 texcoord;
    }
